"""
Per-symbol SL/TP tuning for Relative Strength Momentum (swing) strategy.

Grid: sl_mult ∈ {1.5, 2.0, 2.5, 3.0}, tp_mult ∈ {3.0, 4.0, 6.0}
Runs each symbol independently; picks the (sl, tp) combo with best PF
(≥1.3, trades ≥ 10). Saves to data/rsm_swing_tunings.json.
"""
...
for l in ["rsm_strategy", "backtest", "data_registry", "root"]:
    logging.getLogger(l).setLevel(logging.CRITICAL)

from scripts.backtest import WalkForwardBacktest

RESULTS_FILE = "data/backtest_portfolio_15m_rsm_swing.json"
OUTPUT = "data/rsm_swing_tunings_500.json"
TIMEFRAME = "15m"
DAYS = 730
...
def run_combo(symbol: str, sl_mult: float, tp_mult: float) -> dict:
    ...
    try:
        bt = WalkForwardBacktest(
            ...
            intraday_mode=False,
            force_strategy="Relative Strength Momentum",
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Per-symbol SL/TP tuning for RSM swing")
